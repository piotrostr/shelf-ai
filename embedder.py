...
class CLIPEmbedder:
    def __init__(self):
        ...
        if not torch.backends.mps.is_available():
            if not torch.backends.mps.is_built():
                logging.warn("MPS not available because the current PyTorch install was not "
                             "built with MPS enabled.")
            else:
                logging.warn("MPS not available because the current MacOS version is not 12.3+ "
    ...
    def embed(self, img: np.ndarray, text: str | None = None) -> Embeddings:
        image = self.preprocess(PILImage.fromarray(
            img)).unsqueeze(0).to(self.device)  # type: ignore
    ...
    def embed_path(self, img_path: str) -> Embeddings:
        try:
            image = self.preprocess(PILImage.open(
                img_path)).unsqueeze(0).to(self.device)  # type: ignore
        except:
